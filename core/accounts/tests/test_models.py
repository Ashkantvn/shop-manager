...
class TestAccountsModels:
    ...
    def test_custom_user_str_method(self, custom_user):
        assert str(custom_user) == (
            f"{custom_user.first_name} {custom_user.last_name}"
        )
    ...
    def test_blacklist_access_token_model_is_created(
            self,
            blacklist_access_token):
    ...
    def test_blacklist_access_token_str_method(
            self,
            blacklist_access_token):
        assert str(blacklist_access_token) == (
            f"Token: {blacklist_access_token.token}"
        )
