...
class TestAccountApp:
    ...
    def test_app_GET_profile_view(self, authenticated_manager):
        """
        Tests that the app profile view
        returns a 200 response with the correct template.
        """
    ...
    def test_app_POST_login_view(self, custom_user):
        """
        Tests that the app login view
        redirects to the profile page after successful login.
        """
        client = Client()
        url = reverse("app-accounts:login")
        ...
        response = client.post(
            url, {"username": custom_user.username, "password": "testpassword"}
        )
    ...
    def test_GET_user_update_view(self, authenticated_manager):
        client = authenticated_manager
        url = reverse(
            "app-accounts:update", args=[authenticated_manager.user.user_slug]
        )

        response = client.get(url)
        assert response.status_code == 200
        assert "accounts/update.html" in [
    ...
    def test_app_GET_logout_view(self, authenticated_manager):
        """
        Tests that the app logout view
